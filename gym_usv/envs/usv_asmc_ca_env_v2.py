...
class UsvAsmcCa2Env(gym.Env):
    ...
    def __init__(self):
        ...
        self.w_action1 = 0.2
        self.c_action1 = 1. / np.power((self.max_action1-self.min_action1)/self.integral_step, 2)
        ...
        self.low_state = np.hstack((self.min_u, self.min_v, self.min_r, self.min_ye, self.min_ye_dot, self.min_chi_ak, self.min_sectors, self.min_action1))
        self.high_state = np.hstack((self.max_u, self.max_v, self.max_r, self.max_ye, self.max_ye_dot, self.max_chi_ak, self.max_sectors, self.max_action1))

        self.min_action = np.array([self.min_action1])
        self.max_action = np.array([self.max_action1])
    ...
    def step(self, action):
        ...
        u, v, r, ye, ye_dot, chi_ak, sectors, action1_last = state[0], state[1], state[2], state[3], state[4], state[5], state[6:31], state[31]
        x, y, psi = position
        ...
        #action_dot1 = (action - action1_last)/self.integral_step
        action_dot1 = (action - 0.0)/self.integral_step
        action1_last = action
        ...
        for i in range(10):
            ...
            u_d = (u_ref - self.min_u_ref)*u_psi + self.min_u_ref
            #u_d = action[0]
        ...
        #Compute reward 
        reward = self.compute_reward(ye_abs, chi_ak, action_dot1, collision)
        ...
        self.state = np.hstack((upsilon[0], upsilon[1], upsilon[2], ye, ye_dot, chi_ak, sectors, action1_last))
    ...
    def reset(self):
        ...
        self.state = np.hstack((upsilon[0], upsilon[1], upsilon[2], ye, ye_dot, psi_ak, sectors, action1_last))
        self.position = np.array([eta[0], eta[1], psi])
        self.aux_vars = np.array([e_u_int, Ka_u, Ka_psi])
        self.last = np.array([eta_dot_last[0], eta_dot_last[1], eta_dot_last[2], upsilon_dot_last[0], upsilon_dot_last[1], upsilon_dot_last[2], e_u_last, Ka_dot_u_last, Ka_dot_psi_last])
    ...
    def compute_reward(self, ye, chi_ak, action_dot1, collision):
        if (collision == False):
            ...
            reward_pf = self.w_y*reward_ye + self.w_chi*reward_chi + self.w_action1*reward_a1 
